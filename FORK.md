...
| [#904](https://github.com/googleworkspace/cli/issues/904) — Token cache entries with no `expires_at` are treated as never expiring, so a stale entry is served forever | Mitigated | `yup-oauth2`'s `is_expired()` defaults a missing `expires_at` to "not expired". `EncryptedTokenStorage::get()` now backdates a missing `expires_at` to the Unix epoch before returning it, so the refresh flow fires; that flow preserves the `refresh_token` and repopulates a real `expires_at` on success, so a poisoned entry self-heals after one refresh (`token_storage.rs`) |
