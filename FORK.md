...
| [#886](https://github.com/googleworkspace/cli/issues/886) — Credentials file is silently deleted when decryption fails, with no error explaining why | Fixed | When `credentials.enc` fails to decrypt (e.g. after a keyring/encryption-key change), it is now renamed to `credentials.enc.unreadable` instead of deleted, and the actual decrypt error plus the rename target are printed. The token cache files (`token_cache.json`, `sa_token_cache.json`) are still deleted outright, since they're re-derivable from a fresh login |
