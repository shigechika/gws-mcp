...
Bug reports and feature requests filed against upstream — some targeted the MCP server before it was removed there, others are shared CLI/auth code this fork happens to diverge on. This fork ports or independently fixes them so they remain useful:
...
| [#882](https://github.com/googleworkspace/cli/issues/882) — `gws auth setup`'s pasted OAuth Client ID/Secret is not trimmed, causing 401 invalid_client at login | Fixed | Trimmed on input in the setup wizard (`setup.rs`, also rejects whitespace-only input) and on load in `load_client_config()` (`oauth_config.rs`), so a value persisted before this fix self-heals on the next `gws auth login` without re-running setup |
...
| [#904](https://github.com/googleworkspace/cli/issues/904) — Token cache entries with no `expires_at` are treated as never expiring, so a stale entry is served forever | Mitigated | `yup-oauth2`'s `is_expired()` defaults a missing `expires_at` to "not expired". `EncryptedTokenStorage::get()` now backdates a missing `expires_at` to the Unix epoch before returning it, so `is_expired()` reports true and `yup-oauth2` re-mints the token non-interactively (a refresh-grant using the credentials file's own refresh token for user auth, or a fresh JWT for service accounts) and stores a real `expires_at` on success — so a poisoned entry self-heals after one round trip (`token_storage.rs`) |
| [#878](https://github.com/googleworkspace/cli/issues/878) — `client_secret.json` without `project_id` silently falls back to ambient gcloud ADC `quota_project_id`, escaping `GOOGLE_WORKSPACE_CLI_CONFIG_DIR` isolation | Mitigated | `get_quota_project()`'s ADC fallback behavior is unchanged, but it now prints a stderr warning (once per process) naming the ADC-sourced project when it's used, so a resulting 403 against an unfamiliar project isn't a silent mystery (`auth.rs`) |
