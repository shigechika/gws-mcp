...
| [#780](https://github.com/googleworkspace/cli/issues/780) — `token_cache.json` retains previous account's access token after `gws auth login` with a different account | Fixed | Delete `token_cache.json` immediately after `save_encrypted()` succeeds in `handle_login_inner()` (`auth_commands.rs`). Stale cache no longer causes API calls to use the old account's token for up to ~1 hour |
| [#781](https://github.com/googleworkspace/cli/issues/781) — OAuth2 URL prompt not displayed in non-TTY environments during `gws auth login` proxy flow | Fixed | Changed `println!` → `eprintln!` in `login_with_proxy_support()` (`auth_commands.rs`). `stderr` is unbuffered so the URL appears immediately before the blocking `TcpListener::accept()` call |
| [#793](https://github.com/googleworkspace/cli/issues/793) — `gmail_send --html` / `gmail_reply --html` emit `text/html`-only MIME, rejected by SpamAssassin `MIME_HTML_ONLY` | Fixed | `finalize_message()` now builds `multipart/alternative` with a tag-stripped `text/plain` fallback per RFC 2046 §5.1.4. When regular attachments are present, wraps in `multipart/mixed` (`helpers/gmail/mod.rs`) |
