...
| [#769](https://github.com/googleworkspace/cli/issues/769) — `+reply`（プレーンテキスト）で、ハードラップされた行にプレフィックスを付けると76文字を超え、引用元本文が壊れる | 対応済 | 引用本文の各行を `> ` プレフィックス付与前に73文字で事前ラップし、プレフィックス後も75文字を超えないようにした。quoted-printable のソフトラップが文中に余計な `> ` を挿入するのを防ぐ（`helpers/gmail/reply.rs`） |
| [#774](https://github.com/googleworkspace/cli/issues/774) — Gmail 添付ファイル取得で、パディング無し base64url データが標準デコーダで読めない | 対応済 | 本文/添付ファイルのデコーダを `URL_SAFE_NO_PAD` ＋ `.trim_end_matches('=')` に変更し、パディング有無どちらの base64url 入力も受理する（`helpers/gmail/mod.rs`） |
| [#438](https://github.com/googleworkspace/cli/issues/438) — Gmail メッセージ本文をプレーンテキストで取り出す helper が無い（エージェントが raw の 10〜60KB API payload をパースする必要がある） | 対応済 | `gmail +read` CLI helper が本文を抽出。同じロジックを `gmail_read` MCP helper tool としても公開し、エージェントは raw MIME payload の代わりにパース済みヘッダ＋デコード済み本文を compact JSON で受け取れる（`mcp_server.rs`, `helpers/gmail/mod.rs`） |
| [#572](https://github.com/googleworkspace/cli/issues/572) — アカウント切り替え時、トークンキャッシュが `GOOGLE_WORKSPACE_CLI_CREDENTIALS_FILE` を無視し、同じスコープに対して前のアカウントのトークンを返す | 対応済 | `EncryptedTokenStorage` がスコープに加えて、非可逆なアカウントフィンガープリント（refresh token / service account identity の SHA-256 短縮ハッシュ）でも各キャッシュエントリを名前空間分けするようにし、別アカウントのトークンが衝突しないようにした（`token_storage.rs`, `auth.rs`） |
| [#904](https://github.com/googleworkspace/cli/issues/904) — `expires_at` が無いトークンキャッシュのエントリが「期限切れなし」扱いになり、古いエントリが永久に使われ続ける | 緩和済 | `yup-oauth2` の `is_expired()` は `expires_at` が無い場合を「期限切れではない」と判定してしまう。`EncryptedTokenStorage::get()` が返却直前に `expires_at` の無いエントリを Unix epoch まで遡らせるよう変更し、`is_expired()` が true を返すようにする。これを受けて `yup-oauth2` は非対話的にトークンを再取得する（ユーザー認証では認証情報ファイル自身の refresh_token を使った refresh grant、サービスアカウントでは新規 JWT 発行）。成功時に正しい `expires_at` が再設定されるため、汚染されたエントリは1回の往復で自己修復する（`token_storage.rs`） |
| [#878](https://github.com/googleworkspace/cli/issues/878) — `project_id` を持たない `client_secret.json` を使うと、`GOOGLE_WORKSPACE_CLI_CONFIG_DIR` による分離を無視して gcloud の ambient ADC の `quota_project_id` へサイレントにフォールバックする | 緩和済 | `get_quota_project()` の ADC フォールバック挙動自体は変更していないが、フォールバックが発生した際に使用した ADC 由来の project 名を stderr へ（プロセスあたり1回）警告出力するようにした。見覚えのない project に対する 403 が原因不明のまま埋もれることを防ぐ（`auth.rs`） |
