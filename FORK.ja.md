...
| [#904](https://github.com/googleworkspace/cli/issues/904) — `expires_at` が無いトークンキャッシュのエントリが「期限切れなし」扱いになり、古いエントリが永久に使われ続ける | 緩和済 | `yup-oauth2` の `is_expired()` は `expires_at` が無い場合を「期限切れではない」と判定してしまう。`EncryptedTokenStorage::get()` が返却直前に `expires_at` の無いエントリを Unix epoch まで遡らせるよう変更し、refresh flow を必ず発火させる。refresh flow は `refresh_token` を引き継ぎ、成功時に正しい `expires_at` を再設定するため、汚染されたエントリは1回の refresh で自己修復する（`token_storage.rs`） |
